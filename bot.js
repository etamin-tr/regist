...
client.on("guildMemberAdd", member => {
  member.roles.add('763769165629751298'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "763717206386409472");
    const register = "<@&763769165629751298>"
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = '々'
  const sunucu = '763716154535182356'
  const kanal = '763792871672250448'
  const rol = '763769165629751298'
...
client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get('763716154535182356'); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var tag = "々"; // Buraya Ekip Tag
  var tagrol = "763719913625223188"; // Buraya Ekip Rolünün ID
  var kanal = "763792871672250448"; // Loglanacağı Kanalın ID
